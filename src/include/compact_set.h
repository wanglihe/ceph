...
public:
  class const_iterator : public iterator_base<typename Set::const_iterator> {
	...
	: iterator_base<typename Set::const_iterator>(s, i) { }
      bool end() const {
	return !this->set->set || this->it == this->set->set->end();
      }
  };
  ...
  class const_reverse_iterator : public iterator_base<typename Set::const_reverse_iterator> {
	...
	: iterator_base<typename Set::const_reverse_iterator>(s, i) { }
      bool end() const {
	return !this->set->set || this->it == this->set->set->rend();
      }
  };
...
template <class T>
inline std::ostream& operator<<(std::ostream& out, const compact_set<T>& m)
{
  for (typename compact_set<T>::const_iterator it = m.begin(); !it.end(); ++it) {
    if (it != m.begin())
      out << ",";
