					  ...
					  int linkunlink, SnapRealm *prealm)
{
  ...
  for (compact_set<snapid_t>::iterator p = cur->dirty_old_rstats.begin(); !p.end(); ++p) {
				       ...
				       snapid_t cfollows)
{
  ...
  while (parent) {
    ...
    if (primary_dn) {
      ...
      for (compact_map<snapid_t,old_rstat_t>::iterator p = parent->dirty_old_rstat.begin(); !p.end(); ++p)
