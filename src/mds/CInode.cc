...
bool CInode::get_dirfrags_under(frag_t fg, list<CDir*>& ls)
{
  ...
  for (compact_map<frag_t,CDir*>::iterator p = dirfrags.begin(); !p.end(); ++p) {
...
void CInode::remove_client_cap(client_t client)
{
  ...
  bool fcntl_removed = fcntl_locks ? fcntl_locks->remove_all_from(client) : 0;
  bool flock_removed = flock_locks ? flock_locks->remove_all_from(client) : 0;
...
int CInode::get_caps_wanted(int *ploner, int *pother, int shift, int mask) const
{
  ...
  if (is_auth())
    for (compact_map<int,int>::const_iterator it = mds_caps_wanted.begin(); !it.end(); ++it) {
      w |= it->second;
      other |= it->second;
                                 ...
                                 MDRequestRef &mdr)
{
  ...
  public:
    ...
    bool _dirfrags(int rval) {
      ...
      for (compact_map<frag_t,CDir*>::iterator p = in->dirfrags.begin(); !p.end(); ++p) {
